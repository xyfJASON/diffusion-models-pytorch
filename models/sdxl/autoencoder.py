...
class Decoder(nn.Module):
    ...
    @staticmethod
    def _make_attn() -> Callable:
        return make_attn

    @staticmethod
    def _make_resblock() -> Callable:
        return ResnetBlock

    @staticmethod
    def _make_conv() -> Callable:
...
class AutoencoderKL(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.encoder = Encoder(attn_type=attn_type, double_z=double_z, z_channels=z_channels, resolution=resolution,
        ...
        self.decoder = Decoder(attn_type=attn_type, double_z=double_z, z_channels=z_channels, resolution=resolution,
                               in_channels=in_channels, out_ch=out_ch, ch=ch, ch_mult=ch_mult,
                               num_res_blocks=num_res_blocks, attn_resolutions=attn_resolutions, dropout=dropout)
        self.regularization = DiagonalGaussianRegularizer()
        self.quant_conv = torch.nn.Conv2d((1 + double_z) * z_channels, (1 + double_z) * embed_dim, 1)
