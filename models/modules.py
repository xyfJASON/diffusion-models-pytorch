...
class ConvNormAct(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, kernel_size: int, stride: int, padding: int,
                 norm: str = None, activation: str = None, init_type: str = None, groups: int = 8):
        super().__init__()
...
class ResBlock(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, embed_dim: int):
        super().__init__()
        self.conv1 = ConvNormAct(in_channels, out_channels, 3, stride=1, padding=1, norm='gn', activation='silu')
        self.proj = nn.Sequential(nn.SiLU(), nn.Linear(embed_dim, out_channels))
        self.conv2 = ConvNormAct(out_channels, out_channels, 3, stride=1, padding=1, norm='gn', activation='silu')
...
class UNet(nn.Module):
    def __init__(self, img_channels: int = 3, img_size: int = 32, dim: int = 64, dim_mults: List[int] = (1, 2, 4, 8)):
        ...
        for i in range(n_stages):
            self.down_blocks.append(
                nn.ModuleList([
                    ResBlock(dims[i], dims[i], embed_dim=time_embed_dim),
                    SelfAttentionBlock(dims[i]) if (img_size // (2 ** i)) <= 16 else nn.Identity(),
                    ResBlock(dims[i], dims[i], embed_dim=time_embed_dim),
        ...
        self.bottleneck_block = nn.ModuleList([
            ...
            SelfAttentionBlock(dims[-1]) if (img_size // (2 ** (n_stages - 1))) <= 16 else nn.Identity(),
            ResBlock(dims[-1], dims[-1], embed_dim=time_embed_dim),
        ])
        ...
        for i in range(n_stages):
            self.up_blocks.append(
                nn.ModuleList([
                    ResBlock(dims[i] * 2, dims[i], embed_dim=time_embed_dim),
                    SelfAttentionBlock(dims[i]) if (img_size // (2 ** i)) <= 16 else nn.Identity(),
                    ResBlock(dims[i], dims[i], embed_dim=time_embed_dim),
    ...
    def forward(self, X: Tensor, T: Tensor):
        assert X.shape[-3:] == (self.img_channels, self.img_size, self.img_size)
