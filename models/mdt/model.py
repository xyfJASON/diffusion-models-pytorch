...
class MDTv2(nn.Module):
    ...
    def __init__(
        ...
        self.de_blocks = nn.ModuleList([
            MDTBlock(hidden_size, num_heads, mlp_ratio=mlp_ratio, num_patches=num_patches, skip=True) for _ in range(decode_layer)
        ])
    ...
    def forward(self, x, t, y, enable_mask=False):
        ...
        """
        if y is None:
            y = torch.full((x.shape[0], ), fill_value=self.num_classes, dtype=torch.long, device=x.device)
